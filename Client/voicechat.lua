local VoiceMode = {
	{ dist = 3, message = "Puheen etäisyys asetettu 3 metriin." },
	{ dist = 8, message = "Puheen etäisyys asetettu 8 metriin." },
	{ dist = 14, message = "Puheen etäisyys asetettu 14 metriin." },
	{ veh = true, dist = 4, func = function(ped) return IsPedInAnyVehicle(ped) end, message = "Puheen etäisyys asetettu ajoneuvon sisälle." },
}
...
function Voice:OnModeModified()
	...
	if modeData then
		...
		if modeData.message then
			...
			notifID = exports['mythic_notify']:DoHudText('inform', modeData.message)
